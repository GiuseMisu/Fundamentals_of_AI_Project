...
def update_state(state, last_action, x, y, model):
    ...
    if model(state, last_action) == (x+1, y): # se l'azione è stata eseguita con successo (non ha incontrato un ostacolo) allora aggiorna lo stato massimo 
        state["x_max"] = x+1
    if model(state, last_action) == (x, y+1): 
        state["y_max"] = y+1
    pass
...
def utility(state):
    ...
    x, y = state["x"], state["y"]
    x_max, y_max = state["x_max"], state["y_max"]
    return abs(x_max - x) + abs(y_max - y) # la funzione utility è proporzionale alla dist tra la posizione dell'agente e quella del goal, dopo prendi l'azione con utility più bassa
    pass
...
def utility_based_action(state, model):
    ...
    # a utility maggiore)
    # suggerimento: usare es4_action_space
    next_state = {}
    for action in es4_action_space:
        next_state[action] = model(state, action)
    best_action = None
    infinity = float("inf")
    best_utility = infinity
    for action in es4_action_space:
        if utility(next_state[action]) < best_utility: #utility funzione con valore restituito che è dir proporzionale alla distanza, quindi io voglio quello con utility più bassa
            best_action = action
            best_utility = utility(next_state[action])
    return best_action
    pass
