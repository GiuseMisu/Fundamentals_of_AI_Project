...
def breadth_first_search( #ricerca in ampiezza
    initial_state,#initial_state: stato iniziale
    goal_test,#goal_test: funzione che ritorna true se lo stato è di goal
    successor_fn,#successor_fn: funzione che ritorna la lista dei successori di uno stato
    cost_fn#cost_fn: funzione che ritorna il costo di un'azione
):
    
    #creo la coda dei nodi da esplorare
    queue = []
    #creo il nodo iniziale
    node = Node(initial_state)
    #aggiungo il nodo iniziale alla coda
    queue.append(node)
    #finchè la coda non è vuota
    while len(queue) > 0:
        #prendo il primo nodo della coda
        node = queue.pop(0)
        #se il nodo è di goal ritorno il nodo
        if goal_test(node.state):
            return node
        #altrimenti espando il nodo
        for action, state in successor_fn(node.state):
            #creo il nodo successore
            new_node = Node(state, node.path_cost + cost_fn(action), action, node)
            #aggiungo il nodo alla coda
            queue.append(new_node)
    #se la coda è vuota ritorno None
    return None
...
def uniform_cost_search(
    ...
    while len(queue) > 0:
        ...
        for action, state in successor_fn(node.state):
            ...
            #aggiungo il nodo alla coda
            queue.append(new_node)
        #ordino la coda in base al costo del percorso
...
def depth_first_search(
    ...
    #finchè la coda non è vuota
    while len(queue) > 0:
        #prendo ULTIMOO nodo della coda
        node = queue.pop()
        #se il nodo è di goal ritorno il nodo
        if goal_test(node.state):
            return node
